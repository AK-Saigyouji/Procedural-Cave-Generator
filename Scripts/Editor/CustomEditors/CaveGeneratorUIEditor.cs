...
public class CaveGeneratorUIEditor : Editor
{
    ...
    // These variables are used to display the module editors in this inspector (by default they don't).
    ...
    {
        string caveFolderPath = IOHelpers.CreateFolder(ROOT_FOLDER, CAVE_FOLDER);

        try
        {
            CreateMeshAssets(cave.transform, caveFolderPath);
            IOHelpers.CreatePrefab(cave, caveFolderPath, PREFAB_NAME);
        }
    ...
    void CreateMeshAsset(Transform component, string path)
    {
        ...
        IOHelpers.CreateAsset(mesh, path, mesh.name);
    }
